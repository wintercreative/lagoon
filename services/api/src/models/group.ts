...
export const Group = (clients: any): GroupModel => {
  ...
  const transformKeycloakGroups = async (
    ...
    let groupsWithGroupsAndMembers: any[] = [];
  ...
  const loadGroupByName = async (
    ...
    const flattenGroups = (groups: any, group: any) => {
  ...
  const updateGroup = async (
    ...
    if (oldGroup.name != newGroup.name) {
      ...
      for (const roleSubgroup of roleSubgroups) {
        await updateGroup({
          id: roleSubgroup.id as string,
          name: R.replace(
            oldGroup.name,
            newGroup.name,
            roleSubgroup.name as string
          )
        });
      }
    }
  ...
  const billingGroupCost = async (groupInput: any, yearMonth: string) => {
    ...
    const groupProjects = (await projectsByGroup(group)) as [Project];
  ...
  const allBillingGroupCosts = async (yearMonth: string) => {
    const allGroups = await loadAllGroups();
    const filterFn = (key: string, val: any) => (group: Group) =>
      (group[key]! as string).includes(val);
    const billingGroups = allGroups.filter(filterFn('type', 'billing'));

    const billingGroupCosts: any[] = [];
    for (let i = 0; i < billingGroups.length; i++) {
      const group = billingGroups[i];
      const costs = await billingGroupCost({ id: group.id! }, yearMonth);
      billingGroupCosts.push(costs);
    }

    return billingGroupCosts
      .map((billingGroup: BillingGroup) => {
        if (billingGroup!.availability === 'STANDARD') {
          billingGroup.projects.map((project: any) => {
