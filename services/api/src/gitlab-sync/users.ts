...
(async () => {
  ...
  for (const user of allUsers) {
    ...
    try {
      await api.addUser(user.email, firstName, lastName, null, user.id);
    } catch (err) {
      if (!R.test(usernameExistsRegex, err.message)) {
        logger.error(
          `Could not sync (add) gitlab user ${user.email} id ${user.id}: ${err.message}`
        );
      } else {
        try {
          ...
          logger.error(
            `Could not sync (update) gitlab user ${user.email} id ${user.id}: ${err.message}`
          );
        }
      }
    }
